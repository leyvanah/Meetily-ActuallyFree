...
export function DeviceSelection({ selectedDevices, onDeviceChange, disabled = false }: DeviceSelectionProps) {
  ...
  const fetchDevices = async () => {
    try {
      ...
      setError(t('failedToLoadAudioDevices'));
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const startAudioLevelMonitoring = async () => {
    try {
      ...
      setError(t('failedToStartMonitoring'));
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h4 className="text-sm font-medium text-gray-900">{t('audioDevicesHeading')}</h4>
        ...
        <div className="space-y-2">
          ...
          >
            <SelectTrigger id="mic-selection" className="w-full">
              <SelectValue placeholder={t('selectMicrophonePlaceholder')} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="default">{t('defaultMicrophoneOption')}</SelectItem>
          ...
          {inputDevices.length === 0 && (
            <p className="text-xs text-gray-500">{t('noMicrophoneDevicesFound')}</p>
          )}

          {/* Audio Level Meters for Input Devices */}
          {showLevels && inputDevices.length > 0 && (
            <div className="space-y-2 pt-2 border-t border-gray-100">
              <p className="text-xs text-gray-600 font-medium">{t('microphoneLevelsLabel')}</p>
              {inputDevices.map((device) => {
        ...
        <div className="space-y-2">
          ...
          >
            <SelectTrigger id="system-selection" className="w-full">
              <SelectValue placeholder={t('selectSystemAudioPlaceholder')} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="default">{t('defaultSystemAudioOption')}</SelectItem>
          ...
          {outputDevices.length === 0 && (
            <p className="text-xs text-gray-500">{t('noSystemAudioDevicesFound')}</p>
          )}
          {isMacOS && outputDevices.length > 0 && (
            <p className="text-xs text-gray-500">
              {t('macosSystemAudioNote')}
            </p>
          )}
      ...
      <div className="text-xs text-gray-500 space-y-1">
        <p>• {t.rich('infoMicrophoneBullet', { strong: (chunks) => <strong>{chunks}</strong> })}</p>
        <p>• {t.rich('infoSystemAudioBullet', { strong: (chunks) => <strong>{chunks}</strong> })}</p>
        {isMonitoring && (
          <p>• {t.rich('infoMicLevelsBullet', { strong: (chunks) => <strong>{chunks}</strong> })}</p>
        )}
        {!isMonitoring && inputDevices.length > 0 && (
          <p>• {t.rich('infoTipBullet', { strong: (chunks) => <strong>{chunks}</strong> })}</p>
        )}
      </div>
    </div>
  );
