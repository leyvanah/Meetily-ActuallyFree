...
export function ModelSettingsModal({
  ...
  const fetchOllamaModels = async (silent = false) => {
    ...
    if (trimmedEndpoint && !validateOllamaEndpoint(trimmedEndpoint)) {
      const errorMsg = t('invalidOllamaEndpoint');
      setError(errorMsg);
      if (!silent) {
    ...
    try {
      ...
      const errorMsg = err instanceof Error ? err.message : t('loadOllamaModelsFailed');
      setError(errorMsg);
  ...
  const loadOpenRouterModels = async () => {
    ...
    try {
      ...
      setOpenRouterError(
        err instanceof Error ? err.message : t('loadOpenRouterModelsFailed')
      );
    } finally {
  ...
  const loadBuiltinAiModels = async () => {
    ...
    try {
      ...
      toast.error(t('loadBuiltinModelsFailed'));
    }
  };
  ...
  const handleSave = async () => {
    ...
    if (modelConfig.provider === 'custom-openai') {
      try {
        ...
        toast.error(t('saveCustomOpenAIFailed'));
        return;
      }
    }
  ...
  const testCustomOpenAIConnection = async () => {
    if (!customOpenAIEndpoint.trim() || !customOpenAIModel.trim()) {
      toast.error(t('enterEndpointAndModelFirst'));
      return;
    }
  ...
  const downloadRecommendedModel = async () => {
    ...
    if (isDownloading(recommendedModel)) {
      toast.info(t('modelAlreadyDownloading', { model: recommendedModel }), {
    ...
    try {
      ...
      const errorMsg = err instanceof Error ? err.message : t('downloadModelFailed');
      console.error('Error downloading model:', err);

      // Check if Ollama is not installed and show appropriate error
      if (isOllamaNotInstalledError(errorMsg)) {
        toast.error(t('ollamaNotInstalledTitle'), {
          description: t('ollamaNotInstalledForDownload'),
          duration: 7000,
          action: {
  ...
  const deleteOllamaModel = async (modelName: string) => {
    try {
      ...
      toast.success(t('modelDeleted', { model: modelName }));
      await fetchOllamaModels(true); // Refresh list
    } catch (err) {
      const errorMsg = err instanceof Error ? err.message : t('deleteModelFailed');
      toast.error(errorMsg);
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-lg font-semibold">{t('modelSettingsTitle')}</h3>
      </div>
      ...
      <div className="space-y-4">
                ...
                <PopoverTrigger asChild>
                  ...
                  >
                    <span className="truncate">
                      {modelConfig.model || t('selectModel')}
                    </span>
        ...
        {modelConfig.provider === 'custom-openai' && (
          <div className="space-y-4 border-t pt-4">
            <div>
              <Label htmlFor="custom-endpoint">{t('endpointUrlRequired')}</Label>
              <Input
                id="custom-endpoint"
            ...
            <div>
              <Label htmlFor="custom-model">{t('modelNameRequired')}</Label>
              <Input
                id="custom-model"
            ...
            <div>
              <Label htmlFor="custom-api-key">{t('apiKeyOptional')}</Label>
              <Input
                id="custom-api-key"
                type="password"
            ...
            <div>
              ...
              >
                <Label className="cursor-pointer">{t('advancedOptions')}</Label>
              ...
              {isCustomOpenAIAdvancedOpen && (
                <div className="space-y-3 pl-2 border-l-2 border-muted mt-2">
                  <div>
                    <Label htmlFor="custom-max-tokens">{t('maxTokens')}</Label>
                    <Input
                  ...
                  <div>
                    <Label htmlFor="custom-temperature">{t('temperature')}</Label>
                    <Input
                  ...
                  <div>
                    <Label htmlFor="custom-top-p">{t('topP')}</Label>
                    <Input
                      id="custom-top-p"
                      type="number"
                      step="0.1"
              ...
              <div className="absolute inset-y-0 right-0 pr-1 flex items-center space-x-1">
                {apiKey?.trim() && (
                  <Button
                    ...
                    title={isApiKeyLocked ? t('apiKeyUnlock') : t('apiKeyLock')}
                  >
        ...
        {modelConfig.provider === 'ollama' && (
          <div>
            ...
            >
              <Label className="cursor-pointer">{t('customEndpointOptional')}</Label>
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              {lastFetchedEndpoint && models.length > 0 && (
                <div className="flex items-center gap-2 text-sm">
                  <span className="text-muted-foreground">{t('usingEndpoint')}</span>
            ...
            {isLoadingOllama ? (
                    ...
                    <Alert className="mb-4">
                      <AlertDescription>
                        {ollamaEndpointChanged
                          ? t('endpointChangedHint')
                          : t('noOllamaModelsHint')}
                      </AlertDescription>
                    </Alert>
                    {!ollamaEndpointChanged && (
                      <div className="space-y-3">
                        ...
                        >
                          {isDownloading('gemma3:1b') ? (
                            <>
                              <RefreshCw className="mr-2 h-4 w-4 animate-spin" />
                              {t('downloadingModel', { model: 'gemma3:1b' })}
                            </>
                          ) : (
                            <>
                              <Download className="mr-2 h-4 w-4" />
                              {t('downloadRecommended', { model: 'gemma3:1b' })}
                            </>
                          )}
                        </Button>
                        ...
                        {isDownloading('gemma3:1b') && getProgress('gemma3:1b') !== undefined && (
                          <div className="bg-white rounded-md border p-3">
                            <div className="flex items-center justify-between mb-2">
                              <span className="text-sm font-medium text-blue-600">{t('downloadingNamed', { model: 'gemma3:1b' })}</span>
              ...
              <ScrollArea className="max-h-[calc(100vh-450px)] overflow-y-auto pr-4">
                {filteredModels.length === 0 ? (
                  <Alert>
                    <AlertDescription>
                      {t('noModelsMatching', { query: searchQuery })}
                    </AlertDescription>
                  </Alert>
                ) : (
                      ...
                      return (
                        ...
                        >
                          <div>
                            ...
                            <span className="text-muted-foreground">{t('modelSizeInline')} </span>
