...
export function ModelManager({
  ...
  useEffect(() => {
    ...
    const initializeModels = async () => {
      try {
        ...
        setError(err instanceof Error ? err.message : t('modelsLoadFailedShort'));
        toast.error(t('modelsLoadFailedTitle'), {
    ...
    const setupListeners = async () => {
      ...
      unlistenComplete = await listen<{ modelName: string }>(
        ...
        (event) => {
          ...
          toast.success(t('modelReadyTitle', { icon: getModelIcon(model?.accuracy || 'Good'), model: displayName }), {
            description: t('modelReadyDescription'),
            duration: 4000
          });
      ...
      unlistenError = await listen<{ modelName: string; error: string }>(
        ...
        (event) => {
          ...
          toast.error(t('modelDownloadFailed', { model: displayName }), {
            description: error,
            duration: 6000,
            action: {
              label: 'Retry',
  ...
  const cancelDownload = async (modelName: string) => {
    ...
    try {
      ...
      toast.info(t('modelDownloadCancelled', { model: displayName }), {
        duration: 3000
      });
    } catch (err) {
  ...
  const downloadModel = async (modelName: string) => {
    ...
    try {
      ...
      toast.info(t('modelDownloadingTitleShort', { model: displayName }), {
        description: t('modelDownloadingDescriptionShort'),
        duration: 5000
      });
      ...
      const errorMessage = err instanceof Error ? err.message : t('downloadFailedShort');
      setModels(prev =>
        prev.map(model =>
  ...
  const selectModel = async (modelName: string) => {
    ...
    const displayName = getDisplayName(modelName);
    toast.success(t('switchedToModel', { model: displayName }), {
      duration: 3000
    });
  };
  ...
  const deleteModel = async (modelName: string) => {
    ...
    try {
      ...
      toast.success(t('modelDeletedNamed', { model: displayName }), {
        description: t('modelDeletedDescription'),
        duration: 3000
      });
      ...
      toast.error(t('modelDeleteFailedNamed', { model: displayName }), {
        description: err instanceof Error ? err.message : t('deleteFailedShort'),
        duration: 4000
      });
    }
  };
  ...
  if (error) {
    return (
      <div className={`bg-red-50 border border-red-200 rounded-lg p-4 ${className}`}>
        <p className="text-sm text-red-800">{t('modelsLoadFailedShort')}</p>
            ...
            {isAvailable && (
              <>
                <div className="flex items-center gap-1.5 text-green-600">
                  ...
                  <span className="text-xs font-medium">{t('modelReady')}</span>
                </div>
                <AnimatePresence>
                  {isHovered && (
                    <motion.button
          ...
          >
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                <span className="text-sm font-medium text-blue-600">{t('modelDownloadingLabel')}</span>
            ...
            <div
              ...
              role="progressbar"
              aria-label={t('downloadingAria', { model: displayName })}
              aria-valuemin={0}
              aria-valuemax={100}
